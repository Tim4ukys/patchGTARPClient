...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow)
{
    ...
    g_pRender->m_eventDraw += [](LPDIRECT3DDEVICE9 pDevice, D3DPRESENT_PARAMETERS* pRParams) {
        ...
        if (g_stateProgram == STATE_PROG::PROGRESSBAR) {
            myAddSpaceItem(5.f);
            ImGui::Text(u8"—татус: %s", g_progressbar.getStatus().m_sStatus.c_str());
            myAddSpaceItem(25.f);
            ...
            ImGui::SameLine();
            ImGui::Text(u8"≈сли вы хотете поддержать проект в финансовом плане,\n"
                        u8"то можете сделать это, задонатив на DonationAlerts");

            const auto buttonText = u8"ѕерейти на сайт";
